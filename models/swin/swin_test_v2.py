...
class SubmissionDatasetMakerConverted(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(arr_list)<98:
            for i in range(98-len(arr_list)):
                arr_list.append(np.zeros((256, 256)))
        data_tensor = create_tensor_from_bands_list(arr_list)
        if self.transform:
            data_tensor = self.transform(data_tensor)
...
def experimental_submission(args):
    ...
    for i, current_id in enumerate(chip_ids):
        # img_tmp = np.clip(transformed_predictions[i], 0, np.max(transformed_predictions[i]))
        agbm_path = osp.join(args.submission_folder_path, f"{current_id}_agbm.tif")
        im = Image.fromarray(transformed_predictions[i])
        im.save(agbm_path)
...
def set_args():
    ...
    parser.add_argument('--submission_folder_path', default=str(osp.join(data_path, "imgs", "test_agbm_v1")), type=str)
