...
import argparse
from models.res_swin_v2 import Res_Swin
from models.efficient_swin import Efficient_Swin
            ...
            for index, s1_index in enumerate(self.s1_bands):
                if s1_index == 1:
                    if osp.exists(osp.join(self.training_feature_path, chip_id, str(month), "S1", f"{index}.npy")):
                        band = np.load(osp.join(self.training_feature_path, chip_id, str(month), "S1", f"{index}.npy"),
                                       allow_pickle=True)
            ...
            for index, s2_index in enumerate(self.s2_bands):
                if s2_index == 1:
                    if osp.exists(osp.join(self.training_feature_path, chip_id, str(month), "S2", f"{index}.npy")):
                        band = np.load(osp.join(self.training_feature_path, chip_id, str(month), "S2", f"{index}.npy"),
                                       allow_pickle=True)
                        arr_list.append(band)

        data_augments = A.Compose([
            A.HorizontalFlip(p=0.5),
            A.VerticalFlip(p=0.5),
            # A.ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.2, rotate_limit=45, p=0.25),
            A.RandomGridShuffle(grid=(5, 5), p=0.75),
            ToTensorV2()
        ])

        choices = list(range(len(arr_list)))
        if len(arr_list) < 98:
            for i in range(98 - len(arr_list)):
                random.shuffle(choices)
                arr_list.append(copy.deepcopy(arr_list[choices[0]]))

        datas = create_tensor_from_bands_list(arr_list).numpy()
        # print("1:",datas.shape)
        augmentations = data_augments(image=datas, mask=label)
        # print("2:",augmentations["image"].shape)
        image = augmentations["image"]
        image = image.permute(1,0,2)
        label = augmentations["mask"]
        label = label.unsqueeze(0)
        return image, label
        # return datas, label_tensor
            ...
            for index, s1_index in enumerate(self.s1_bands):
                if s1_index == 1:
                    if osp.exists(osp.join(self.training_data_path, chip_id, str(month), "S1", f"{index}.npy")):
                        band = np.load(osp.join(self.training_data_path, chip_id, str(month), "S1", f"{index}.npy"),
                                       allow_pickle=True)
                        arr_list.append(band)
            for index, s2_index in enumerate(self.s2_bands):
                if s2_index == 1:
                    if osp.exists(osp.join(self.training_data_path, chip_id, str(month), "S2", f"{index}.npy")):
                        band = np.load(osp.join(self.training_data_path, chip_id, str(month), "S2", f"{index}.npy"),
                                       allow_pickle=True)
                        arr_list.append(band)
        if len(arr_list) < 112:
...
def prepare_dataset_tiff(training_ids_path, training_features_path, training_labels_path, S1_band_selection,
                         S2_band_selection):
    with open(training_ids_path, newline='') as f:
        reader = csv.reader(f)
        patch_name_data = list(reader)
    ...
    new_dataset = SegmentationDatasetMakerTIFF(training_features_path, training_labels_path, id_month_list,
                                               S1_band_selection,
                                               S2_band_selection)
    return new_dataset, (S1_band_selection.count(1) + S2_band_selection.count(1))
...
def load_model(args):
    ...
    version_dir = list(os.scandir(log_folder_path))[0]
...
def set_args():
    ...
    model_identifier = f"res_swin_v2_{s1_bands_indicator}_{s2_bands_indicator}"
    parser.add_argument('--model_identifier', default=model_identifier, type=str)
    parser.add_argument('--encoder_name', default="res_swin_v2", type=str)
