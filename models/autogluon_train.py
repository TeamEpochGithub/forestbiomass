...
def prepare_dataset_testing(args):
    ...
    new_dataset = ChainedSegmentationSubmissionDatasetMaker(
...
def train(args):
    ...
    band_channel_count = args.S1_band_selection.count(1) + args.S2_band_selection.count(
        1
    )
...
def load_model(args):
    ...
    band_segmenter_model = select_segmenter(
        args.band_segmenter_name,
        args.band_encoder_name,
        args.band_encoder_weights_name,
        band_channel_count,
    )

    month_segmenter_model = select_segmenter(
        args.month_segmenter_name,
        args.month_encoder_name,
        args.month_encoder_weights_name,
        month_channel_count,
    )

    model = ChainedSegmenter(
        band_model=band_segmenter_model,
    ...
    for index, id in enumerate(patch_names):
        ...
        for month in range(0, 12):
            ...
            if args.S1_band_selection.count(1) >= 1:
                ...
                S1_bands = [
                    x for x, y in zip(S1_bands, args.S1_band_selection) if y == 1
                ]
                bands.extend(S1_bands)
            ...
            if args.S2_band_selection.count(1) >= 1:
                ...
                S2_bands = [
                    x for x, y in zip(S2_bands, args.S2_band_selection) if y == 1
                ]
                bands.extend(S2_bands)
